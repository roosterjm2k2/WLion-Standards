...
class WLion_Sniffs_WhiteSpace_SuperfluousWhitespaceSniff implements PHP_CodeSniffer_Sniff
{
	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
	{
		...
		if ($tokens[$stackPtr]['code'] === T_OPEN_TAG) {
			...
			*/
			...
			/*
				Check for multiple blanks lines anywhere.
			*/
			...
			/*
				Check for empty line not followed by a comment
			 */
			if ($tokens[($stackPtr - 1)]['line'] < $tokens[$stackPtr]['line'] && $tokens[($stackPtr - 2)]['line'] === $tokens[($stackPtr - 1)]['line']) {
				// This is an empty line and the line before this one is not
				//  empty, so this could be the start of a multiple empty
				// line block.
				$next  = $phpcsFile->findNext(T_COMMENT, $stackPtr, NULL, TRUE);
				$lines = $tokens[$next]['line'] - $tokens[$stackPtr]['line'];
				if ($lines > 1) {
					$error = "Extra Empty Line or Missing Comment";
					$data = array();
					$phpcsFile->addError($error, $stackPtr, 'EmptyLines', $data);
				}
			}
